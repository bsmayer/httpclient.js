...
describe('HttpClientResponse', () => {
  ...
  it('should intercept error and not throw', async () => {
    const interceptor = HttpClientInterceptors.create().useErrorInterceptor(() => ({
      message: 'let it pass',
    }));
  ...
  it('should intercept the error and throw', async () => {
    ...
    await HttpClientBuilder.create('http://api.github.com')
      .useInterceptors(interceptor)
      .client()
      .path('users', 'throw', '401')
      .get()
  ...
  it('should intercept the response and get the main object', async () => {
    const interceptors = HttpClientInterceptors.create().useResponseInterceptor(
      (response: any) => response.return
    );
  ...
  it('should retry 3 times before intercepting the error', async () => {
    const interceptors = HttpClientInterceptors.create().useErrorInterceptor(() => {
      throw new Error('we should catch this error');
    });
